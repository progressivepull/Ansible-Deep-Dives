...
# Different Templating Systems (${{}} vs. {{}} in a Playbook)

In Ansible playbooks, {{ }} and ${{ }} look similar, but they come from different templating systems and are used in different contexts.

In 2025, the double curly brace syntax **{{ }}** remains the native standard for **Jinja2 templating** within Ansible. This system is evaluated by the Ansible control node before a task is sent to the target machine, allowing you to inject variables, facts, and the results of previous tasks into your playbooks. Because Ansible uses Jinja2 as its primary engine, these braces are used for almost all dynamic data substitution, such as referencing **{{ ansible_hostname }}** or applying filters like **{{ my_var | default('value') }}**.

In contrast, the **\${{ }}** syntax is not a native Ansible feature and typically appears when Ansible is integrated with external platforms like GitHub Actions or Azure DevOps. In those environments, **\${{ }}** is used for "expression evaluation" at the orchestration layer—the platform processes these variables before the Ansible playbook even begins to run. For example, a GitHub Action might use **\${{ github.token }}** to pass a secret into an Ansible command-line argument, but the Ansible playbook itself would never see those specific dollar-sign braces; it only sees the final value they produced.

Mixing these two in the same file can lead to confusion because they serve different masters: **{{ }}** is for Ansible’s internal logic, while **\${{ }}** is for the external "wrapper" or CI/CD pipeline. In 2025, a common error for developers is accidentally using the **\${{ }}** syntax inside an Ansible task, which will cause a YAML parsing error or a literal string failure because Ansible's Jinja2 engine does not recognize the dollar sign prefix as a valid variable delimiter. Always use standard **{{ }}** for variables defined in host_vars, group_vars, or registered results.
